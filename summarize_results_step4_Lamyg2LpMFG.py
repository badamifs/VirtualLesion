...
for subject in subjects_sorted:
    ...
    def warp_mni_to_img(target, warp, phy_coords):
        print "Warping" + subject
    ''' Warp (physical-space) MNI-space coords to a target image. '''
        phy_coords = np.array([[-24.,-1.,-21.]]).T
        warp = nb.load(os.path.join(path, subject, '/MNINonLinear/xfms/standard2acpc_dc.nii.gz'))
        target = nb.load(os.path.join(path_saveing, subject, '/dwi_MD.nii.gz'))
    ...
    def xform_sl(subject, sl_file, img_coords=True):
        out_dir = os.path.join(path_saveing, subject)
        warp = nb.load(os.path.join(path, subject, '/MNINonLinear/xfms/acpc_dc2standard.nii.gz'))
        mni = nb.load('/usr/share/fsl/data/standard/MNI152_T1_1mm.nii.gz')
        #fa = nb.load(os.path.join(out_dir,'dwi_FA.nii.gz'))
        ref = nb.load(os.path.join(path, subject, '/T1w/T1w_acpc_dc_restore_1.25.nii.gz'))
    ...
    def make_rois(subject, roi_names, roi_dilation=None, qa=False):
        out_dir = os.path.join(path_saveing, subject)
        sub_dir = os.path.join(path, subject)
