...
for subject in subjects_sorted:
    ...
    if os.path.isfile(os.path.join(path_saveing, subject, 'Lamyg2LpMFG_Strength_of_Evidence.txt')) == False:
        print "    Strength of Evidence does not exist, start calculation"
        
        if os.path.isfile(os.path.join(path_saveing, subject, 'Lamyg2LpMFG_neighborhood.nii.gz')) == True and os.path.isfile(os.path.join(path_saveing, subject, 'Lamyg2LpMFG_LiFE_rmse_with_path.nii.gz')) == True and os.path.isfile(os.path.join(path_saveing, subject, 'Lamyg2LpMFG_LiFE_rmse_without_path.nii.gz')) == True and os.path.isfile(os.path.join(path, subject, 'T1w/Diffusion/data.nii.gz')) == True:
            print "    All necessary files exist, continue"
            
            directory_output = os.path.join(path_saveing, subject)

            mask = load_nifti(path_saveing + subject + "/Lamyg2LpMFG_neighborhood")
            mask = mask.astype(bool)

            with_path = load_nifti(path_saveing + subject + "/Lamyg2LpMFG_LiFE_rmse_with_path")
            without_path = load_nifti(path_saveing + subject + "/Lamyg2LpMFG_LiFE_rmse_without_path")
            ...
            try:
                ...
                tv.write(os.path.join(path_saveing, subject, 'Lamyg2LpMFG_optimized.trk'), optimized_save,  hdr_mapping=hdr)
            except:
