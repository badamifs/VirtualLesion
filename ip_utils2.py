...
def xform_coords(xform, coords):
    '''coords must be a 3xN array'''
    c = np.dot(xform, np.vstack((coords, np.ones((1,coords.shape[1])))))
    return c[:3,...]
...
def plot_brain(m, cmap, clip, txt_color='w', outfile=None, label=None, fig=None, overlay=None, overlay_cmap='autumn', overlay_clip=None):
    if outfile!=None:
    ...
    if overlay!=None:
        if overlay_clip!=None:
           overlay_clip = [overlay.min(),overlay.max()]
        ol = plt.imshow(overlay, cmap=overlay_cmap, alpha=0.7, vmin=overlay_clip[0], vmax=overlay_clip[1])
    plt.axis('off')
    if label:
        # colorbar
        c = getattr(cm,cmap)
        cmap = np.array([c(i)[0:3] for i in xrange(c.N)])
        cbax = add_subplot_axes(fig, im.axes, [.3,0.07, 0.4,0.08])
        plt.imshow(np.tile(cmap,(12,1,1)).transpose((0,1,2)), axes=cbax)
        cbax.set_yticks([])
        cbax.set_xlabel(label, color=txt_color, size=24)
        ...
        cbax.xaxis.set_tick_params(labelsize=24, labelcolor=txt_color)
    plt.tight_layout()
    if outfile:
...
def compare_brains(ni_files, sl, outfile=None, clip=None, label=None, cmap='gray', fig=None):
    ims = []
    txt_color = 'w'
    for i in xrange(len(ni_files)):
        d,xform,clip = prep_brain(ni_files[i], clip)
        #cur_sl = xform_coord(np.linalg.inv(xform), sl).round()
        if len(d.shape)>3 and d.shape[3]==3:
            mask = d.sum(axis=3)
            r,crop = autocrop(d[...,0], mask, pad=0)
            r = np.rot90(r[:,sl[i],:])
            g,crop = autocrop(d[...,1], mask, pad=0)
            g = np.rot90(g[:,sl[i],:])
            b,crop = autocrop(d[...,2], mask, pad=0)
            b = np.rot90(b[:,sl[i],:])
            ims.append(np.dstack((r,g,b)).astype(np.uint8))
            cmap = None
        else:
            im,crop = autocrop(d, pad=0)
            cur_sl = [cur_sl[i]-crop[i][0] for i in xrange(3)]
            ims.append(np.rot90(im[:,sl[i],:]))
    im = hstack_centered(ims)
    plot_brain(im, cmap, clip, txt_color, outfile, label, fig)

def show_brain(ni_file, sl=[0,0,0], outfile=None, clip=None, label=None, cmap='gray', fig=None, bg='', overlay_file=None, overlay_cmap='autumn', overlay_clip=None):
    from scipy import ndimage

    pad = 10
    d,xform,clip = prep_brain(ni_file, clip)
    sl = xform_coord(np.linalg.inv(xform), sl).round()
    sl = [min(sl[i],d.shape[i]) for i in xrange(3)]
    txt_color = 'w'
    if len(d.shape)>3 and d.shape[3]==3:
        mask = d.sum(axis=3)
        r,crop = autocrop(d[...,0], mask, pad=pad)
        sl = [sl[i]-crop[i][0] for i in xrange(3)]
        r = three_axis(r,sl)
        g,crop = autocrop(d[...,1], mask, pad=pad)
        g = three_axis(g,sl)
        b,crop = autocrop(d[...,2], mask, pad=pad)
        b = three_axis(b,sl)
        ...
        if 'w' in bg:
            ...
            txt_color = 'k'
    if overlay_file:
        od,oxform,oclip = prep_brain(overlay_file)
        overlay,crop = autocrop(od, mask=d, pad=pad)
        overlay = three_axis(overlay, sl)
        overlay = np.ma.masked_where(overlay<overlay_clip[0], overlay)
    else:
        overlay = None
    plot_brain(m, cmap, clip, txt_color, outfile, label, fig, overlay=overlay, overlay_cmap=overlay_cmap, overlay_clip=overlay_clip)
